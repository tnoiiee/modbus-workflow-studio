...
describe('O1-C critical UX source contracts (issues 1, 2, 5, 6, 10, 13, 14)', () => {
  ...
  it('version is v1.3.0-dev.2 across canonical sources (issue 13)', () => {
    const version = read(['version.ts']);
    expect(version).toContain("'1.3.0-dev.2'");
    ...
    expect(rootPkg.version).toBe('1.3.0-dev.2');
    expect(clientPkg.version).toBe('1.3.0-dev.2');
  });
