...
describe('O1-C critical UX source contracts (issues 1, 2, 5, 6, 10, 13, 14)', () => {
  ...
  it('version is v1.4.0-dev.1 across canonical sources (issue 13)', () => {
    const version = read(['version.ts']);
    expect(version).toContain("'1.4.0-dev.1'");
    ...
    expect(rootPkg.version).toBe('1.4.0-dev.1');
    expect(clientPkg.version).toBe('1.4.0-dev.1');
  });
