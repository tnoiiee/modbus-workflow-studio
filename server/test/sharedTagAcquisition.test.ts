...
import { createOverviewElement, patchOverviewBinding, emptyOverviewHistory, pushOverviewHistory, undoOverviewHistory, redoOverviewHistory, canUndoOverview, canRedoOverview } from '../../client/src/lib/overviewElements.js';
...
describe('independent bounded Shared Tag acquisition owner', () => {
  ...
  it('runtime leaves Definition/Page/index/mapping/Control-state bytes, revision and actual Draft history helpers untouched', async () => {
    const f = setup(), m = f.map(); f.config.put(m);
    const variable = f.catalog.create({ sourceType: 'WORKFLOW_VARIABLE', workflowId: '11111111-1111-4111-8111-111111111111', name: 'V', dataType: 'Number', capability: 'MONITOR_ONLY' });
    const element = createOverviewElement('NUMERIC_LABEL', { id: 'e', x: 0, y: 0 });
    element.binding = patchOverviewBinding(element.category, element.binding, { source: definitionIdentity(variable), dataType: 'Number' });
    const control = createOverviewElement('SWITCH', { id: 'preview', x: 100, y: 100 });
    const pages = new OverviewPageManager(f.dir), created = pages.create({ name: 'Protected' });
    const page = pages.update(created.id, { expectedRevision: created.revision, elements: [element, control] });
    const controls = new OverviewControlStateStore(f.dir); controls.set(page.id, control.id, true);
    // Use the real client history helpers; this is not a mounted Browser integration test.
    const savedElements = [element, control], edited = [{ ...element, x: 123 }, control];
    const undo = undoOverviewHistory(pushOverviewHistory(emptyOverviewHistory(), savedElements), edited)!;
    const draft = undo.value, history = undo.history;
    expect(canRedoOverview(history)).toBe(true); expect(canUndoOverview(history)).toBe(false);
    const draftBefore = JSON.stringify({ draft, history });
    const files = ['source-definitions.json', 'overview-pages.json', `overview-pages/${page.id}.json`, 'shared-tag-acquisition.json', 'overview-control-states.json'];
    const bytes = () => files.map(file => fs.readFileSync(path.join(f.dir, file)));
    const hashes = () => bytes().map(buffer => createHash('sha256').update(buffer).digest('hex'));
    const before = bytes(), shaBefore = hashes(), revision = page.revision, modifiedAt = page.modifiedAt;
    f.service.start(); await vi.advanceTimersByTimeAsync(500);
    expect(f.store.get(m.sourceId)).toMatchObject({ quality: 'GOOD', value: 42 });
    f.connection.manual = true; f.connection.generation++; f.connection.runtime.actualState = 'disconnected'; f.service.reconcile();
    await vi.advanceTimersByTimeAsync(500);
    expect(f.store.get(m.sourceId)).toMatchObject({ quality: 'DISCONNECTED', lastGoodValue: 42 });
    expect(bytes()).toEqual(before); expect(hashes()).toEqual(shaBefore);
    expect(pages.get(page.id)).toMatchObject({ revision, modifiedAt });
    expect(new OverviewPageManager(f.dir).get(page.id)).toMatchObject({ revision, modifiedAt });
    expect(controls.get(page.id, control.id)?.value).toBe(true);
    expect(JSON.stringify({ draft, history })).toBe(draftBefore);
    const redo = redoOverviewHistory(history, draft)!; expect(redo.value).toEqual(edited); expect(canUndoOverview(redo.history)).toBe(true);
  ...
  it.each(['success', 'error'])('delayed old %s after Disconnect cannot replace DISCONNECTED or last-good', async outcome => {
    const f = setup(), m = f.map(); f.config.put(m); f.service.start(); await flush();
    const good = f.store.get(m.sourceId)!; expect(good.quality).toBe('GOOD');
    let resolve!: (value: Buffer) => void, reject!: (error: Error) => void;
    f.connection.request.mockImplementationOnce(() => new Promise((yes, no) => { resolve = yes; reject = no; }));
    await vi.advanceTimersByTimeAsync(100); expect(f.connection.request).toHaveBeenCalledTimes(2);
    f.connection.manual = true; f.connection.generation++; f.connection.runtime.actualState = 'disconnected'; f.service.reconcile();
    const disconnected = f.store.get(m.sourceId)!;
    await vi.advanceTimersByTimeAsync(1000);
    if (outcome === 'success') resolve(readResponse(99)); else reject(Error('late controlled failure'));
    await flush(); await vi.advanceTimersByTimeAsync(500);
    expect(f.store.get(m.sourceId)).toEqual(disconnected);
    expect(disconnected).toMatchObject({ quality: 'DISCONNECTED', lastGoodValue: 42, lastGoodReceiveTimestamp: good.lastGoodReceiveTimestamp });
    expect(f.connection.request).toHaveBeenCalledTimes(2); expect(f.connection.connect).not.toHaveBeenCalled();
  });
});
