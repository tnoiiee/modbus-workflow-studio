...
app.delete('/api/overview-pages/:id',(q,r)=>{if(!overviewIdSchema.safeParse(q.params.id).success)return r.status(400).json({error:'Invalid Overview page ID'});try{overviewPageManager.delete(q.params.id);r.json({ok:true})}catch(error){r.status(overviewStatus(error)).json({error:(error as Error).message})}});
app.patch('/api/overview-pages/:pageId/elements/:elementId/control-state',(q,r)=>{
  ...
  const parsed=overviewControlStateSchema.safeParse(q.body);
  if(!parsed.success)return r.status(400).json({error:'Valid control-state payload is required'});
  try{
    const result=overviewPageManager.updateElementControlState(q.params.pageId,elementId,parsed.data);
    r.json(result);
  }catch(error){
    const status=overviewStatus(error);
    r.status(status).json({error:(error as Error).message,currentRevision:status===409?overviewPageManager.get(q.params.pageId)?.revision:undefined});
  }
});
...
app.use('/api',(_q,r)=>r.status(404).json({error:'API endpoint not found'}));const dist=path.resolve('../client/dist');app.use(express.static(dist));app.get('*',(_q,r)=>r.sendFile(path.join(dist,'index.html')));server.listen(PORT,HOST,()=>console.log(`MODBUS WORKFLOW STUDIO v1.2.12 on http://${HOST}:${PORT}`));
