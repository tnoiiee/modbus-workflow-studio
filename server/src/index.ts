...
const ds=new JsonStore<DeviceConfig[]>(path.join(DATA,'devices.json'),[]),wsStore=new JsonStore<Workflow>(path.join(DATA,'workflow.json'),defaultW),audit=new JsonStore<unknown[]>(path.join(DATA,'audit.json'),[]);let devices=ds.load(),workflow=wsStore.load();if(workflow.mode==='LIVE_ARMED')workflow.mode='LIVE_LOCKED';workflow.running=false;const workflowManager=new WorkflowManager(DATA,workflow);workflowManager.downgradeArmed();let activeWorkflowId=workflowManager.first().id;workflow=workflowManager.first();let runningWorkflowId:string|undefined;const runtime:Record<string,RuntimeValue>={},engineMemory=new Map<string,unknown>(),outputPending=new Set<string>(),lastWrittenCommand=new Map<string,unknown>(),outputInitialized=new Set<string>(),lastWriteAt=new Map<string,number>(),manualTriggerTimers=new Map<string,NodeJS.Timeout>(),connections=new Map<string,DeviceConnection>(),traffic:unknown[]=[];let pollers=new Map<string,NodeJS.Timeout>();
const app=express();app.use(cors());app.use(express.json({limit:'256kb'}));const server=http.createServer(app),wss=new WebSocketServer({server,path:'/ws/live'});const broadcast=(type:string,data:unknown,workflowId?:string)=>{const s=JSON.stringify({type,workflowId,data});for(const c of wss.clients)if(c.readyState===1)c.send(s)};
const conn=(d:DeviceConfig)=>{let c=connections.get(d.id);if(!c){c=new DeviceConnection(d);c.on('state',()=>broadcast('devices',listDevices()));c.on('traffic',x=>{traffic.unshift(x);traffic.length= Math.min(traffic.length,5000);broadcast('traffic',x)});connections.set(d.id,c)}return c};const listDevices=()=>devices.map(d=>({...d,runtime:connections.get(d.id)?.runtime??{desiredState:'disconnected',actualState:'disconnected',queueLength:0,activePollers:0,averageResponseTime:0,timeoutCount:0}}));
...
app.use('/api',(_q,r)=>r.status(404).json({error:'API endpoint not found'}));const dist=path.resolve('../client/dist');app.use(express.static(dist));app.get('*',(_q,r)=>r.sendFile(path.join(dist,'index.html')));server.listen(PORT,HOST,()=>console.log(`MODBUS WORKFLOW STUDIO v1.2.10 on http://${HOST}:${PORT}`));
