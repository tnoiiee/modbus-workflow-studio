import { AcquisitionConfig } from './acquisitionConfig.js';
import { SharedTagAcquisition } from './sharedTagAcquisition.js';
import { TagRuntimeStore } from './tagRuntime.js';
import { registerAcquisitionRoutes } from './acquisitionRoutes.js';
import { definitionReferences, definitionReferenceBatch } from './definitionReferences.js';
...
const acquisitionConfig = new AcquisitionConfig(DATA, definitionCatalog, () => devices);
const sharedTagAcquisition = new SharedTagAcquisition(acquisitionConfig, new TagRuntimeStore(), id => connections.get(id));
// Mutations invalidate acquisition only; no browser lifecycle or runtime delivery wiring.
app.use((q, r, next) => {
  if (['POST', 'PUT', 'PATCH', 'DELETE'].includes(q.method) && (q.path.startsWith('/api/source-definitions') || q.path.startsWith('/api/devices'))) {
    r.once('finish', () => { if (r.statusCode < 400) sharedTagAcquisition.reconcile(); });
  }
  next();
});
registerAcquisitionRoutes(app, acquisitionConfig);
sharedTagAcquisition.start();
registerDefinitionRoutes(app, definitionCatalog, identity => definitionReferences(overviewPageManager, identity), sources => definitionReferenceBatch(overviewPageManager, definitionCatalog, sources));

const sendLiveSnapshot=(client:import('ws').WebSocket)=>{queueLiveEvent(client,'hello',{version:'1.4.0-dev.6',revision:liveRevision,reliability:{wsMaxMessages:RELIABILITY.wsMaxMessages,wsMaxBytes:RELIABILITY.wsMaxBytes,reconnectBaseMs:RELIABILITY.wsReconnectBaseMs,reconnectMaxMs:RELIABILITY.wsReconnectMaxMs,reconnectJitter:RELIABILITY.wsReconnectJitter}});queueLiveEvent(client,'devices',listDevices());queueLiveEvent(client,'workflow-list',workflowList());const selected=workflowManager.get(activeWorkflowId);if(selected)queueLiveEvent(client,'workflow',{...selected,running:runtimeManager.isRunning(selected.id)},selected.id)};
...
app.use('/api',(_q,r)=>r.status(404).json({error:'API endpoint not found'}));const dist=path.resolve('../client/dist');app.use(express.static(dist));app.get('*',(_q,r)=>r.sendFile(path.join(dist,'index.html')));server.listen(PORT,HOST,()=>console.log(`MODBUS WORKFLOW STUDIO v1.4.0-dev.6 on http://${HOST}:${PORT}`));
